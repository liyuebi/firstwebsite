...
{	
	...
	else 
	{
		...
		$res = mysql_query("select * from CreditTrade where IdxId='$issueId'");
		...
		$res1 = mysql_query("select * from ClientTable where UserId='$respondentId'");
		...
		while ($i < 10) {
			...
			$res1 = mysql_query("select * from Complaint where CompId='$compId'");
			if ($res1) {
		...
		$res2 = mysql_query("insert into Complaint (CompId, Type, RelatedIdx, RelatedIssueId, ComplainantId, CompNickname, CompPhoneNum, RespondentId, RespNickname, RespPhoneNum, IssueDesc, IssueTime, Status)
		...
		if (!$res2) {
			echo json_encode(array('error'=>'true','error_code'=>'6','error_msg'=>'发起投诉失败，请稍后重试！','sql_error'=>mysql_error()));
			return;
		}
	}
